...
//            + "(:isLowCalorie IS NULL OR (p.categoryName = '음료' AND p.kcal < 20) OR (p.categoryName != '음료' AND p.kcal < 40)) AND "
//            + "(:isSugarFree IS NULL OR p.sugar <= 1) AND "
//            + "(:isLowSugar IS NULL OR p.sugar <= p.servingSize * 0.05) AND "
//            + "(:isLowCarb IS NULL OR (p.carbohydrate >= p.servingSize * 0.11 AND p.carbohydrate <= p.servingSize * 0.20)) AND "
...
//            + "(:isLowFat IS NULL OR ((p.categoryName = '음료' AND p.fat <= 1.5) OR (p.categoryName != '음료' AND p.fat <= 3.0))) AND "
